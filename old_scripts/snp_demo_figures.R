...
single_metrics_df$table_type <- ifelse(single_metrics_df$x.value == 3195.79, "static","dynamic")


## need to work out method for combining plots
ggplot() + 
    ...
    geom_boxplot(data = single_metrics_df[single_metrics_df$table_type == "static",],
                 aes(x = class, y = y.value)) + 
    facet_grid(table_type~metric, scale = "free_x")  +
